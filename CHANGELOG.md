...
- **A verdict is attributed to what produced the finding.** `quorum feedback` (CLI and MCP) resolved a finding's id and rule but wrote `model` and `in_diff` from flags nobody passes and every skill field as `None`: 86% of the feedback corpus has no model (99% of the last two weeks) and no row names an axis. The review now records, per finding, `in_diff`, the axis (`skill_name`, `skill_version`, `manifest_sha256`) and the category (schema v6 on `review_finding_ids`), and `ReviewLog::attribution_for` joins the review's model; a verdict recorded without `--model`, `--in-diff` or `--category` takes them from the resolved finding (a flag still wins). The MCP `feedback` tool now resolves the finding id by file and title like the CLI does. Legacy rows read as unknown. The benchmark-harness half of the same review item is filed as #619. Item 6 of the signal-to-noise plan (the gpt-6-astra review of the pipeline, September 2026).

- **Hidden out-of-diff findings are kept, and the changed function rescues its own model findings.** With `--diff-file`, a finding outside the changed lines is still hidden by default, but no longer destroyed: it is recorded with the review like any other (after the project's suppression rules), so `quorum feedback` resolves a verdict on it and the hidden set can be labelled; `stats --join-health` therefore counts hidden findings among a review's recorded findings. And a model finding anchored inside the enclosing function of a changed line is shown: measured once locally on the #589 benchmark diff, the two outside-hunk findings that were real bugs (the wrong unlock in `feedback.rs`, the removed trait import in `pipeline.rs`) sat 23 and 36 lines from a hunk inside the changed function, while the noise (an `unsafe` block) sat 1,477 lines away in another. Rule findings are never rescued: an unbounded rescue surfaced two deliberate `block_on` calls 1,700 lines from the hunk on this change's own review. Function nodes are known for Rust, Python, TypeScript (declarations and methods), Bash and C++; elsewhere the rescue is a no-op. Item 4 of the signal-to-noise plan (the gpt-6-astra review of the pipeline, September 2026).

- **An incomplete review says so end to end.** When a skill axis fails (the model answers with prose, runs out of room, or the call fails), the summary line now ends with `N of M skill axes failed (review incomplete)`, `--json` carries `_meta.incomplete` (`axes_failed`, `axes_total`, and one `file: axis/model (class or reason)` label per failed cell, sorted), and the hard-error early exit (an unreadable input plus no findings) writes the same payload instead of a bare `[]`, `quorum report` renders that ahead of the findings so the PR comment cannot read as a clean "No findings.", and the exit-code floor of 1 now applies on the `--github-pr` path too (it returned the unfloored code). A truncated answer is logged as `truncated` instead of `network_error`, through a typed error the client raises and the executor classifies in one place, and the audit log writes `parse_error_class`/`failure_reason` as `null` rather than omitting them. The CI workflow still masks the exit code with `|| true`; the artifact now carries the signal instead. Item 5 of the signal-to-noise plan.

- **The integrator no longer invents a confidence.** Every finding reaching the skill integrator had `confidence == None` (the axes' output schema never asked for one), and `confidence_of` substituted 0.5, so a single finding always came out at 0.5 and two axes agreeing on a defect came out at 0.75, a number nobody had said; `integrator_decisions.jsonl` recorded the 0.5 as if the model had reported it (2,268 rows, every input exactly 0.5). Now the axes ask the model for `confidence` (0.0 to 1.0, omit if unsure), the integrator reads it when nothing computed one, the noisy-or runs over known values only, an all-unknown cluster stays unknown (and is never below the floor), and the log writes `null` for an unknown input or output. A model-reported value ranks and is logged but cannot suppress: only a computed confidence can put a cluster below the floor, since nothing on the CLI shows an integrator-suppressed finding (filed as #616). `stats --integrator --json` now emits `avg_output_confidence: null` for a group with no known output confidence, and an older binary reading a log with `null` rows counts them as parse errors rather than crashing. Item 8 of the signal-to-noise plan.
...
- **Diff-first review input for the axes.** With `--diff-file`, each axis used to receive the whole file (a 260 KB `main.rs` cost about 64k tokens per axis per review) and never the diff itself: the parser keeps only new-side line ranges, so deleted lines were invisible. The skill matrix now receives a focused view per file: the enclosing function of every changed range (or the range with 20 lines of context when no function contains it), every line prefixed with its absolute line number, omitted regions marked, and the file's unified-diff hunks after the code so the model sees the actual edit, deletions included. If the view would keep more than 60% of the file, or the diff never mentions the file, or the file appears only in deletion hunks (no new-side lines to focus on), the whole file goes as before. `focus::focus_source` and `focus::hunks_for_file` are pure and unit-tested; `tests/diff_first_input.rs` pins at the wire that the request carries the changed function and the hunks and omits an untouched function far from the change, and that a review without a diff still sends the whole file.
  ...
  Measured on the original #589 diff (five files, gpt-5.6, two axes): 239k tokens in became 55k, 110 s became 52 s, and the review found two of the five ground-truth defects exactly (whitespace-only lines rewritten empty; every lock error reported as contention) plus a partial on lock ordering. Whole-file input had found none of the three in-diff defects in two runs.

  Two things are new for readers of the output. The model can now see deleted lines, so a finding about a deletion has no new-side line to anchor to; whichever line it cites may be hidden by the out-of-diff default (the #562 semantics). And the layout of a focused view is explained in the `<code_to_review>` metadata line (`"view": {"kind": "focused", "numbered_lines": true, ...}`) and the base system prompt, never inside the code fence, because the model is told never to follow text found there. Library API: `skill_executor::execute_matrix` and `expand_matrix` take `&[ReviewFile]` instead of `(path, sha, code)` tuples, and `CellSpec` gains a `focus` field; both are consumed only by the binary. `hunks_for_file` reads each hunk's extent from its `@@` header, so plain `diff -u` output with no `diff --git` separators is handled, and hunk text is bounded by bytes as well as lines.

- **`--diff-file` hides findings outside the diff by default.** The feedback corpus puts LLM findings inside the changed lines at 88% precision and outside them at 7%, and CI has no feedback corpus to suppress the outside ones with, so every PR review carried 17 to 26 findings that were about code the PR did not touch. Findings stamped `in_diff: false` are now dropped after classification and before project suppressions, on both the sequential and parallel paths, through one function (`pipeline::hide_out_of_diff`). The summary line reports the count ("N outside the diff hidden (run with --show-out-of-diff to see them)"). `in_diff: None` is kept: no diff, or a deletion-only hunk (#562), is unknown, not outside. `--show-out-of-diff` restores the old behaviour. Mutation: making the filter also drop `None` fails `hide_out_of_diff_drops_only_known_outside`; wiring the flag to `false` fails `diff_file_hides_out_of_diff_findings_unless_asked`.

- **Default code-mode axes are correctness and security.** A six-axis review of a five-file diff on 2026-09-14 cost 712k tokens and produced 66 findings; architecture, simplicity, performance and testing-antipatterns produced 40 of them and none of the bugs. The four remain available through `--axes`, and `--mode` audits can list them. Expected effect on a routine review: about a third of the tokens and wall time.
...
- **Rubric-gated severity bumps (Track A, #187).** Calibrator post-hoc severity bumps now require rubric support: CRITICAL needs an explicit keyword (rce / code execution / auth bypass / credential leak / data corruption / guaranteed crash / data loss) with word-boundary matching to avoid false hits like "rce" inside "brute force"; HIGH requires the finding's category be on an allowlist (security, correctness, logic, concurrency, reliability, robustness, error-handling, validation). Eliminates calibrator-driven CRITICAL inflation on rubric-violating bumps measured at ~30-40% pre-fix on the 7-challenge dub-flow corpus. Default-on; `QUORUM_RUBRIC_GATE=off|0` bypasses. The `QUORUM_DISABLE_CALIBRATOR` ablation knob now works on both `calibrate` and `calibrate_with_index` (was only the latter pre-fix). Independent gpt-5.5 review caught a category blocklist→allowlist design fault during implementation; word-boundary matching added after a regression test caught "rce" inside "brute force" letting an MD5 password-hashing finding through.

- **`severity_change_reason` trace metadata (Track B, #189).** Every `CalibratorTraceEntry` now carries one of five reasons explaining why severity did or didn't change: `Boosted` / `BoostBlockedByGate` / `Disputed` / `BoostWeightTooLow` / `NoMatch`. Trace-only schema addition; pre-Track-B trace files keep parsing via `#[serde(default, skip_serializing_if = "Option::is_none")]`. Wired at every calibrator decision branch in both `calibrate` and `calibrate_with_index`. Bundled CRITICAL keyword tightening (codex review #3) replaced over-broad `"secret leak"` / `"data loss"` with active-voice variants (`"durable data loss"`, `"leaks credentials"`, `"exfiltrates secrets"`, `"leaks the password"`, ...) — speculative phrasings like "logs may leak secrets" no longer pass the gate. CR-driven `disable_calibrator: Option<bool>` field added to `CalibratorConfig` with `calibrator_disabled()` helper so tests can use config injection instead of `std::env::set_var` (eliminates parallel-test race per `docs/TEST_STRATEGY.md`); production env-var path preserved as fallback. In-branch self-review caught an ablation-knob ordering bug where `QUORUM_DISABLE_CALIBRATOR` was checked AFTER the empty-index `NoMatch` trace emission in `calibrate_with_index`; order swapped so disable always short-circuits first.
