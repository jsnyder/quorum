...
pub struct ReviewOpts {
    ...
    /// (hidden by default when the diff has added or changed lines: corpus
    /// precision on them is 7% vs 88% inside; a deletion-only diff hides
    /// nothing; a model finding inside a changed function is shown anyway,
    /// where quorum knows the language's function nodes; hidden ones are
    /// still recorded, so `quorum feedback` can reach them)
    #[arg(long)]
...
pub struct FeedbackOpts {
    ...
    /// unknown (omitted). On the Human path, omitted inherits what the
    /// review recorded for the resolved finding; pass it when you mean
    /// something else (a finding outside the hunks that you fixed in-branch
    /// anyway). The External path (--from-agent) records what the agent
    /// supplied and inherits nothing.
    #[arg(long)]
