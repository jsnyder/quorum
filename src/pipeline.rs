...
pub struct FileReviewResult {
    ...
    /// Findings hidden because `in_diff == Some(false)`, outside the changed
    /// functions, and the caller did not ask for them. Counted in the summary
    /// line and recorded with the review so a verdict can still reach them.
    pub hidden_out_of_diff: Vec<Finding>,
...
mod tests {
    ...
    /// #520: `judge: required` must be enforced whether or not a judge ran.
    ///
    /// The retain that withholds unjudged findings lives inside
    /// `judge_findings`, which only runs when `judge_enabled` is true --
    /// so the guard sat downstream of the very flag whose absence it
    ...
    async fn narrow_rules_still_emit_when_the_speculative_ones_are_withheld() {
        ...
        let source = "fn strip(line: &str) -> &str {\n    &line[6..]\n}\n";
        let tree = parser::parse(source, Language::Rust).unwrap();
        let config = PipelineConfig::default();
        let ast_ctx = AstContext {
            tree: &tree,
    ...
    fn hide_out_of_diff_drops_only_known_outside() {
        ...
        assert_eq!(
            hidden[0].title, "out",
            "the hidden finding comes back, not a count"
        );
    ...
    fn hide_out_of_diff_rescues_model_findings_in_changed_functions() {
        ...
        let hidden = hide_out_of_diff(&mut findings, &[(10, 60)]);
        assert_eq!(
            findings
                .iter()
