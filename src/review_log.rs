...
mod tests {
    ...
    /// A JSONL row written before #405 must still deserialize.
    ///
    /// #555 deleted `skills_used` / `skill_findings` /
    /// `integrator_findings_out`, so the original assertions here are gone,
    /// but the property that mattered is unchanged: old rows on disk load
    /// without the fields they lack. Rows written *with* those keys also load
    /// -- serde ignores unknown fields by default, and `ReviewRecord` has no
    /// `deny_unknown_fields` -- so the JSONL archives that carry them are
    /// still readable.
    #[test]
    fn legacy_review_record_deserializes() {
        let legacy = r#"{"run_id":"01ABC","timestamp":"2026-01-01T00:00:00Z","quorum_version":"0.1","repo":null,"invoked_from":"tty","model":"gpt","files_reviewed":1,"lines_added":null,"lines_removed":null,"findings_by_severity":{"critical":0,"high":0,"medium":0,"low":0,"info":0},"tokens_in":0,"tokens_out":0,"duration_ms":0}"#;
        let rec: ReviewRecord = serde_json::from_str(legacy).expect("legacy load");
        assert_eq!(rec.run_id, "01ABC");
    }

    /// A row carrying the deleted keys still loads, ignoring them.
    #[test]
    fn record_with_deleted_skill_keys_still_deserializes() {
        let with_keys = r#"{"run_id":"01XYZ","timestamp":"2026-01-01T00:00:00Z","quorum_version":"0.1","repo":null,"invoked_from":"tty","model":"gpt","files_reviewed":1,"lines_added":null,"lines_removed":null,"findings_by_severity":{"critical":0,"high":0,"medium":0,"low":0,"info":0},"tokens_in":0,"tokens_out":0,"duration_ms":0,"skills_used":["security-reviewer"],"skill_findings":{"security-reviewer":3},"integrator_findings_out":4}"#;
        let rec: ReviewRecord =
            serde_json::from_str(with_keys).expect("rows with the old keys must still load");
        assert_eq!(rec.run_id, "01XYZ");
    }
