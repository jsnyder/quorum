...
Each skill axis (correctness, security, or any `--axes` selection) is one LLM call per file. The user message is `<review_context>` (framework docs, hydration scoped to the changed lines, injected project context, historical verdicts, rendered by `review::render_context_sections`, the same renderer the legacy single-prompt path uses), then `<code_to_review>` (the whole file, or with `--diff-file` a focused view: enclosing functions of the changed ranges with absolute line numbers plus the file's hunks; see `src/focus.rs`), then `<skill_instructions>` (the axis manifest), then the output schema. The file-stable sections lead so that every axis reviewing a file sends the same prefix (system message, context, code) and an OpenAI-compatible provider can serve it from its prompt cache; per file the axes run sequentially inside `execute_matrix`, so the second axis finds the prefix the first one just wrote. The base system prompt (`src/skill_base_system_prompt.txt`) is identical for every call: it explains the ordering, primes the read with what every axis looks for (so the code is not read cold before the axis instructions arrive), explains the focused-view metadata, and states the status of `<review_context>` as reference data. The review summary line reports prompt tokens served from cache when the provider returns `cached_tokens`.
